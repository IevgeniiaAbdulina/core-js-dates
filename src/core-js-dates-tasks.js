...
function getDayName(date) {
  ...
  return dayNames[birthday.getDay()];
}
...
function getNextFriday(date) {
  ...
  return new Date(
    day.setDate(day.getDate() + ((7 - day.getDay() + 5) % 7 || 7))
  );
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    getDateTime(date) >= getDateTime(start) &&
    getDateTime(date) <= getDateTime(end)
  );
}
...
function getNextFridayThe13th(date) {
  ...
  let thirteenDate = date.setDate(13);
  let next13Friday = new Date();
  let isFriday = false;

  if (day < 13) {
    thirteenDate = date.setDate(13);
    isFriday = date.getDay() === 5;

    if (isFriday) {
      next13Friday = new Date(thirteenDate);
    } else {
      for (; !isFriday; ) {
        thirteenDate = new Date(date.setMonth(date.getMonth() + 1));
        isFriday = thirteenDate.getDay() === 5;
      }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startParam = period.start.split('-').reverse().join('-');
  const endParam = period.end.split('-').reverse().join('-');

  const twoDigitDate = (digit) => {
    return digit < 10 ? `0${digit}` : digit;
  };

  const workDayFormat = (date) => {
    const year = date.getFullYear();
    const month = date.getMonth() + 1;
    const day = date.getDate();

    return `${twoDigitDate(day)}-${twoDigitDate(month)}-${year}`;
  };
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays && startDate <= endDate; i += 1) {
      workSchedule.push(workDayFormat(startDate));
